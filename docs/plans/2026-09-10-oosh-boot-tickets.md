...
| **Runner-level test isolation** | Ticket 1 (2026-09-15) made shared-config isolation *opt-in* per file — the next file that forgets `test.suite.config.isolate` rewrites the site-wide tier again, and only `test.path` has a canary. Ticket 3's handoff `testresult.env` still lands in the **shared** `~/config` (two users' runs clobber each other; `results.clear` even carries a branch for "another user's file is unlinkable"), and a `core` file with no score never fails the run. Fix as one card: the runner isolates every `core`/`extended` child by default (one `mktemp -d` per run, seeded by copying `user.env`/`oosh.env`/`log.env` so reads work and writes go nowhere; `platform` excluded), keeps its handoff in that per-run dir, and returns 1 for `core` when any file produced no score. Small; schedule before T7 so T7's tests are trustworthy. | [board backlog § D](2026-09-15-board-backlog.md) |
