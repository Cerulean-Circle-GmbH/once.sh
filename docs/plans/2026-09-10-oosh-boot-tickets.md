...
| **Portability assumptions in TEST-HARNESS code** | Distinct from production portability (cards 1-3, delivered): three failures on 2026-09-17 had the same shape and in **all three the code under test was correct** — the test reported working code broken, with a confident and specific message. `T-SSH-BACKUP-LIST` used `grep -c -- '-one$\|-two$'`; `\|` is a **GNU** BRE extension, BSD grep counted 0, and it reported `user.ssh.backup.list` broken on macOS. Eight fixture assertions compared a raw `mktemp` path against a **resolved** one (macOS `/tmp` → `/private/tmp`, plus `$TMPDIR`'s trailing slash) and reported `oo.mode`, `oo.mode.base.get` and `odocker workspace.init` broken. `T50` ran its probe through `script -qec`, which does not guarantee which shell runs the command — under dash it died on `source: not found`, never sourced `log` at all, and reported `log`'s D4 guard broken in **every** container for a day. Measured surface remaining in `test/` + `test.suite`: **74** un-canonicalised `mktemp -d`, **7** `stat -c` with no BSD fallback (18 others already have one, so the convention exists and is simply unenforced), **3** files with GNU-only `\|`, **4** `readlink -f`, **3** `script -qec` without an explicit shell, **3** `sed -i` with no `''` suffix, **1** `grep -P`. Deliverable in the tree's own idiom — a sweep with a `# portability-exception: <reason>` marker, verdict on stdout, rc 1 on a violation, exactly like `path.validate` / `this.anchor.validate` / `test.completion.audit` — plus fixing whichever sites it flags as **reachable**. Scope note: most of the 74 are latent (a fixture path only matters when something resolves it — only 7 ever failed), so mark rather than churn. Out of scope: production portability, and `test.hiveMind` (`TEST_CATEGORY=extended`, known failures, owns 3 of the `\|` files). | [research § 11](../research/2026-09-17-shared-tier-leak-macos.md) |
