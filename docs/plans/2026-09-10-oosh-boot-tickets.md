...
| 7 | **T10** — remove the `log.session.env` touch-guard from `this` | ⚪ **Open** (added 2026-09-23) | Follow-on of 2e2e6a6: `log` owns the per-user session file, but hosts installed earlier kept the `. $OOSH_USER_CONFIG_PATH/log.session.env` chain line in the shared `log.env`. `config init.user` (→ `oo update`) now strips it (`private.config.log.env.migrate`, test.config T88). Once every host has run `oo update`, delete the guard at the top of `this` (the `[ -f … log.session.env ] \|\| { mkdir …; : > … }` block) and its comment. Verify first with `grep -c log.session.env ~/config/log.env` → 0 on each host. |
| 8 | **T11** — unpaired apostrophes break `c2` signature parsing tree-wide | ⚪ **Open** (added 2026-09-24) | Found while fixing `ogit` completion (boss review of method vs parameter completion). `c2` reads parameter names from the signature line through `line.unquote`; an unpaired `'` in the docstring (`the user's ~/config`) swallows the rest, the parameters are misread and the method never completes — `oo user.fix <Tab>` parses its parameter as `shared` and never offers usernames, `config init.user` as `match`. 37 docstrings outside `ogit` (oo, config, os, osshLayout, ossh, …). `ogit`'s were fixed with the new `c2 signature.validate <script>`. To do: run it over `IN_SCOPE_SCRIPTS`, reword each docstring (or teach the parser a lone apostrophe — then with tests), then make `c2 signature.validate` a `test.completion.audit` gate so it cannot regress. Same check also flags a second cause: an extra `#` inside the description (`path.validate` → `declares [treeRoot] c2 reads []`). Related, found the same day: `c2 completion.discover … c2 <method>` (class given as `c2`, the symlink name) parses no c2 method at all — only the `ng/c2` path works; pre-existing. Rule: [oosh-architecture.md § Completion Function Rules](../oosh-architecture.md#completion-function-rules) item 6. |
| 9 | **T12** — `replace commit` makes every file it commits executable | ⚪ **Open** (added 2026-09-24) | Found while syncing the ogit plan's docstrings with `replace line`: `replace.commit` (replace:44) runs `chmod -x "$FILE"` before the swap and `chmod +x "$FILE"` after it unconditionally — built for scripts, so a committed `.md` came out `100755` (restored by hand). It has happened before: this tracker itself went `100644 → 100755` in `4bef93a` — reset it to 644 as part of the fix. Fix: record the mode before the swap (`[ -x "$FILE" ]`) and set `+x` only when it was executable; red-first test in `test/test.replace` (T-REPLACE-COMMIT-KEEPS-MODE: a non-executable file stays non-executable, an executable one stays executable). Independent of T11 (that is c2's signature parser). |
...
| ~~**`config.save` F4 + F5 + F6 — card 2**~~ 🔍 **In Review (2026-09-17)** | `config.save` runs `config.validate` on what it wrote and discards the verdict (`>/dev/null 2>&1` + `warn.log`) — the guard caught the macOS corruption on the first install and nobody heard it. `config init.env`, the repair `config validate required` advertises, leaves `oosh.env` EMPTY and one variable worse, because it persists an environment the broken files failed to populate. And the required-variable table's *re-derivation* column has no consumer. **DELIVERED 2026-09-17** (`2e999ea`…`6c5a121`), and the three decisions were taken as asked: F4 reports rather than halts, the third column became executable, and `config.init.env` became the real repair rather than a new verb. Six `private.config.derive.*` getters give each required variable ONE named accessor, called from the repair and from `config.validate.required` so the drift check and the repair cannot disagree. `config.init.env` now backs up all THREE files (backing up only `user.env` is exactly why `OOSH_PM` was lost), re-derives only variables EMPTY in this shell, names a deriver that returns nothing rather than dropping it silently, and REFUSES TO WORSEN — it re-validates after the save and restores if the missing set grew. F4's verdict goes to **stderr**, not stdout, because `$(user get basehome)` captures `config.save`'s stdout (`user:1702-1708`) and a stray line there once corrupted a generated git config. Two corrections found on the way: a deriver must have NO SIDE EFFECTS (the first `OOSH_PM` one called `oo pm.discover`, which runs `sudo apt-get update` and sat on a password prompt), and T83's first form asserted `config.save`'s stdout was EMPTY when it has always carried `console.log` — narrowed to "the VERDICT must not be on stdout", which is the assertion that actually protects the caller. | [research § 6](../research/2026-09-17-config-save-bash32.md) |
| ~~**`test.odocker` fails in every container**~~ 🔍 **In Review (2026-09-17)** | Six assertions asserted that THIS MACHINE has the EAMD.ucp `DockerWorkspaces` tree — a different repository (`donges/eamd.ucp.git`); no Dockerfile ships with oosh at all. `os platform.test` runs `test.suite core 1` four times per gate, so it was 6 × 4 failures. **Delivered, but NOT as the card proposed.** The card asked for the same `[ -d "$ODOCKER_WORKSPACES" ]` skip the `DOCKER_AVAILABLE` assertions use. Grounding it killed that design: `expect.pass` bumps both counters and `expect.fail` only one (`test.suite:770-782`), so **a skip is indistinguishable from a pass** — on every container, CI runner and fresh install those six would have reported green while asserting nothing. Instead they split: `T-WS-LIST` in `core` tests the enumerator against a fixture built with `odocker workspace.init` (its first non-test caller) and passes anywhere, including the one-level glob and the no-Dockerfile decoy that nothing pinned before; and `test/test.platform.odocker.workspaces.invariant` keeps the host-readiness question, reading the must-pass list from `defaults/platforms.env` instead of the four hand-copied names. Also: a dangling section header with no assertion under it since it was written, and two stale line references. | [research § 14](../research/2026-09-17-config-save-bash32.md) |
...
| **Five hand-rolled `ODOCKER_WORKSPACES` save/restore pairs** | `test/test.odocker` saves and restores the workspaces root by hand at `:589-599`, `:768`, `:847`, `:862-922` and `:933-1017`, and the 2026-09-17 fixture block makes a sixth shape. `test.suite.config.isolate` (`test.suite:841`) is the tree's only isolation primitive and knows nothing but `CONFIG_PATH`/`CONFIG_FILE`/`CONFIG`. A sibling `test.suite.workspaces.isolate` is the DRY answer, but it is a new primitive in the test framework and did not belong in a ticket about six failing assertions. | — |
...
| ~~**Portability assumptions in TEST-HARNESS code**~~ 🔍 **In Review (2026-09-17)** | Distinct from production portability (cards 1-3, delivered): three failures on 2026-09-17 had the same shape and in **all three the code under test was correct** — the test reported working code broken, with a confident and specific message. `T-SSH-BACKUP-LIST` used `grep -c -- '-one$\|-two$'`; `\|` is a **GNU** BRE extension, BSD grep counted 0, and it reported `user.ssh.backup.list` broken on macOS. Eight fixture assertions compared a raw `mktemp` path against a **resolved** one (macOS `/tmp` → `/private/tmp`, plus `$TMPDIR`'s trailing slash) and reported `oo.mode`, `oo.mode.base.get` and `odocker workspace.init` broken. `T50` ran its probe through `script -qec`, which does not guarantee which shell runs the command — under dash it died on `source: not found`, never sourced `log` at all, and reported `log`'s D4 guard broken in **every** container for a day. Measured surface remaining in `test/` + `test.suite`: **74** un-canonicalised `mktemp -d`, **7** `stat -c` with no BSD fallback (18 others already have one, so the convention exists and is simply unenforced), **3** files with GNU-only `\|`, **4** `readlink -f`, **3** `script -qec` without an explicit shell, **3** `sed -i` with no `''` suffix, **1** `grep -P`. Deliverable in the tree's own idiom — a sweep with a `# portability-exception: <reason>` marker, verdict on stdout, rc 1 on a violation, exactly like `path.validate` / `this.anchor.validate` / `test.completion.audit` — plus fixing whichever sites it flags as **reachable**. Scope note: most of the 74 are latent (a fixture path only matters when something resolves it — only 7 ever failed), so mark rather than churn. Out of scope: production portability, and `test.hiveMind` (`TEST_CATEGORY=extended`, known failures, owns 3 of the `\|` files). **Delivered 2026-09-17, and the measured surface was wrong in both directions.** `test.suite portability.validate [<treeRoot>]` is the sweep, sibling of `path.validate`, with `T-PORTABILITY-TREE` in `test/test.test.suite` as the gate and five more cases proving it can fail, honours its marker, and does not fire on the correct forms. **The first draft over-reported**: `grep -E` lines where `\|` is an escaped *literal* pipe (3 sites), `stat -c` whose BSD fallback sits on the **continued** line (6 sites, so the 7 "unpaired" ones were 1), `date -r … || date -d …` where the BSD form is already first, `pgrep -P` caught by a `grep -P` rule with no word anchor, and `sed -i` inside an `expect.pass` **message string**. Each became a rule the sweep understands rather than a marker in a file. **And under-reported**: the `\|` count was 3 *files*, not 3 sites — 14 real ones across `test.c2`, `test.config`, `test.hiveMind`, `test.oo` and `test.otmux`. All 22 genuine violations fixed; `test.hiveMind` was in scope after all (its 6 `\|` sites are mechanical, and it scores 115/138 before and after, so its known failures are untouched). The 74 `mktemp` estimate was 118, which is why the sweep has **two severities**: `mktemp` is an `advisory` — reported with a per-file tally, does **not** fail the gate — and `T-PORTABILITY-ADVISORY` pins that, because a rule that is permanently red is a rule nobody reads. `test.suite.fixture.make` is the conversion target when you are in a file anyway. Two prerequisites: `oo method.new` could not resolve a dotted script name (`test.suite.portability.validate` resolved to `test`, a **directory**) — fixed with longest-existing-prefix resolution plus `T-METHOD-NEW-DOTTED-SCRIPT`; and `test.suite` had no `### new.method` marker. Documented in [docs/test-suite.md](../test-suite.md). | [research § 11](../research/2026-09-17-shared-tier-leak-macos.md) |
| **The `mktemp` advisories — and the rule that over-counts them** | The follow-up the portability card deliberately did **not** do, and it is **two** pieces of work, the first of which is a defect in the sweep itself. **(a) The `mktemp` rule has no command-position anchor.** It is the bare pattern `mktemp`, so it matches the word inside a message string: **36** of the 119 reported advisories are `\|\| { create.result 1 "mktemp failed"; return $(result); }` and its variants — not uses of `mktemp` at all. This is the *same* false-positive class that was fixed for `sed -i` during the portability card (`expect.pass "…sed -i pattern"` was being flagged) and left unfixed here, so the published tally overstates the surface by **43%**. Fix the pattern the way `sed -i`'s was fixed — `(^\|[\|;&(]\|\$\()[[:space:]]*mktemp` — and pin it with a case that plants `create.result 1 "mktemp failed"` and expects **0** advisories. Do this FIRST: until it is done, every number anyone works from is wrong. **(b) Then the conversion.** Measured surface after (a): **83** real calls — **70** `mktemp -d` (the risky kind: a directory whose path gets compared) and **12** plain `mktemp` (mostly harmless). Concentration is `test.oo` 51, `test.config` 19, `test.test.suite` 17, `test.promote` 9, then a tail of ones. Target is `test.suite.fixture.make <label>` (mktemp -d + `private.this.path.canonical`), already used in 15 places; on Linux it is a **no-op**, so converting cannot change behaviour on the dev box — and equally, the dev box is not where a mistake would show. **Scope, which is the actual decision and is NOT taken here:** only **6** files both create raw fixtures and contain a resolving call (`path.canonical` / `absolutePath` / `pwd -P` / `git worktree` / `realpath` / `readlink`) — `test.oo`, `test.this`, `test.install`, `test.ossh`, `test.odocker`, `test.platform.shared.config.invariant`. Those 6 are the defensible unit: it is where the next failure comes from, it is bounded, and it is verifiable on the macOS VM. The remaining ~77 are churn today. Three honest options for them, in the boss's gift: **(i)** leave the advisory as-is and convert opportunistically when in the file anyway — the status quo, and defensible, since the gate already stops new violations of the six hard rules and this class has produced exactly one incident; **(ii)** convert everything in one pass and promote `mktemp` to a `violation`, which is the only route that ever ends the class; **(iii)** convert the 6, keep the advisory for the rest. Whichever is chosen, the verification is the macOS VM, not the host — a `/private/tmp` mismatch is invisible on Linux by construction. **Why this is latent and not urgent:** the 8 assertions that actually failed this way (blaming `oo.mode`, `oo.mode.base.get` and `odocker workspace.init` on a clean Sequoia install, all three of which were fine) are already fixed. Nothing is red. **(a) DELIVERED 2026-09-17** (`2da63be`), at the user's direction, because the fix is about the sweep's credibility rather than about fixtures. Both rules now share a command-position anchor; the backtick is in its class for the one `` `mktemp` `` site and `$(` covers the other 81. Verified as a SET, not a count: the rule's matches and a hand-built list of real invocations agree **exactly** — 82 each, no misses, no extras. **The 83 estimated above was itself one too high**, having counted a comment line, so the real surface is **82** and every quoted count is corrected in `test.suite` and `docs/test-suite.md`. `T-PORTABILITY-ADVISORY-STRING` pins it, red first. **(b) remains open and remains the boss's call** — nothing above has changed except that its numbers are now trustworthy. | [research § 18](../research/2026-09-17-shared-tier-leak-macos.md) |
| ~~**alpine_3_19 fails `core` — three defects, all pre-existing, all busybox**~~ 🔍 **In Review (2026-09-18)** | `os platform.test alpine_3_19` is **FAIL** as of 2026-09-18, 3 real failures in all four user gates. **Bisected in the container, not reasoned**: at `93a9a12` (before 2026-09-17's work) alpine scored **845/835/10**; at `e9da6ce` it scores **859/855/4**. The same three failures are present at both commits, so none is a regression — the 2026-09-17 work removed six other alpine failures on the way past. **(1) + (2) `OS_CMD_USER_DEL` is never filled.** Alpine has `deluser`/`delgroup` (busybox) and **no** `userdel`/`groupdel`/`useradd`. `private.user.init`'s heal (`user:1673-1678`, from `4ee09d3e`, 2026-04-24) probes **only** `userdel`, while its three siblings all carry a busybox fallback — `USER_ADD` → `adduser`, `USER_MOD` → `addgroup`, `GROUP_ADD` → `addgroup`. `USER_DEL` is the one of four without one, so `T-USER-INIT-SELF-HEAL: fills missing OS_CMD_USER_DEL` and the `os.commands.env not re-saved with complete set` check that depends on it both fail. This is not only a test problem: **`user.delete` has no working command on alpine at all**, and `test.user` logs `user.delete.linux … ENOENT 127` repeatedly, which also makes that file slow there. Fix is one `elif` mirroring the `USER_MOD` shape — but check what `deluser`'s argument order actually is before copying the pattern, since busybox `deluser` and shadow `userdel` differ. **(3) The canary fires on `test.completion.audit`**: `log.env` grows 62 → 89 bytes during that file, the runner names it, restores the tier and fails the run. It is NOT the known `log` hardcoded-`~/config` card — that one is about `result.txt`/`error.txt`. Two measured facts narrow it: the file **isolates correctly** (`test/test.completion.audit:18`, before sourcing anything), and run **standalone it does not write at all** — `log.env` stays 62. It only writes inside a `core` sequence, so an earlier file leaves state that makes the 29-file run persist something extra. `test.completion.audit:86` sources every script in the tree with `source "$scriptPath" completion.discover`, which is the obvious suspect and the thing to instrument first. Alpine-only: ubuntu_24_04, debian_12, almalinux_9 and macOS all pass the same check. **DELIVERED 2026-09-18** (`a0d3a5b`) — **alpine_3_19 PASS (test=0 root=0 oosh-user=0 bash-user=0)**, and it took **all three** failures, not the two the card scoped. The card treated the canary (3) as a separate defect; it was the SAME root cause, and `823d50a` had already written down why: while the OS_CMD cache is incomplete, `private.user.init` takes its persist branch — `config save os.commands` + `oo pm.discover` — on **every** invocation, and that cascade is what writes the shared tier. `test.completion.audit` sources every script in the tree, so it re-triggered the persist dozens of times; run standalone it wrote nothing because the cache was already warm. One root cause, three symptoms — which is also why "it only writes inside a core sequence" was the clue, not a curiosity. **Bigger than the card said**: `OS_CMD_USER_DEL` was not merely unfilled, it was **write-only** — `private.user.delete.linux` never read the cache at all, running its own probe with a hardcoded `userdel -rf`. One accessor, `private.user.delete.command.get`, now answers for both, created with `oo method.new`. The flag is part of the value (as for darwin's `sysadminctl -deleteUser`) because the spellings differ: `userdel -rf USER` against `deluser --remove-home USER`. `<sbinDir>` is a real parameter — /usr/sbin is off a non-root PATH on Debian — and also the only way to exercise the busybox branch on a shadow host: **the first version of the test stubbed PATH alone, the absolute /usr/sbin/userdel answered anyway, and the busybox case passed for the wrong reason**, which is the same shape as the bug and was caught before commit. | — |
...
| 2026-09-17 | **The macOS gate's first catch, researched before anything was touched** (`339370a`). A fresh Sequoia install shipped a `~/config/oosh.env` that no shell could source: every login printed `OOSH_MODE: command not found`. Cause: on **bash 3.2** `declare -p` with no arguments prints `set`-style output **plus every function body**, and the `sed` meant to rewrite `declare -x` into `export` never matched — so `config.save` wrote prose rows into an env file. Six faults recorded, F1–F6, split into three cards so each could be gated separately. Research first and STOP, per the standing rule; nothing was fixed in this commit. |
| 2026-09-17 | **`oo method.new` can create a private method** (`287073f`) — the prerequisite, taken before card 1 because the project mandates template-driven method creation and card 1 needed four new `private.config.*` methods. The tool split `private.x.y` at the wrong dot and aimed the insertion at a script named `private`. |
| 2026-09-17 | **Card 1 delivered — F1/F2/F3** (`13f75dc`…`0710fe8`). `config.save` now harvests variable **NAMES** and emits `export NAME=value` itself, so what `declare -p` is allowed to print no longer matters — the fix removes the dependency rather than patching the parse. The `${var^^}` uppercase fallback is gone: on bash 3.2 it is a FATAL "bad substitution" that **exits the shell**, so `config add` was killing the login it ran in. `result.save` writes the named RESULT variables instead of every line matching the string `RESULT` (its defect was the value-substring grep, NOT `declare -px`, which is fine on 3.2 — a claim corrected in the research doc after measuring). T75–T78 inject a simulated bash-3.2 stream so the regression is caught on bash 5. **`test.platform.shared.config.env.invariant` is the gap that let this ship**: its sibling asserted the config DIRECTORY — symlink, owner, mode — and nothing whatever about what was INSIDE the files. |
| 2026-09-17 | **`test.odocker` stops asserting that this machine has another repo** (`d60cf69`…`df6d57b`). Six assertions required the EAMD.ucp `DockerWorkspaces` tree — a different repository; no Dockerfile ships with oosh at all — so `os platform.test` failed them 6 × 4 users, every gate. Delivered **not as the card proposed**: the card wanted the `DOCKER_AVAILABLE` skip, but `expect.pass` bumps both counters and `expect.fail` only one, so **a skip is indistinguishable from a pass** and all six would have reported green while asserting nothing. Split instead into a fixture test in `core` and a platform invariant. Ubuntu gate 7 failures → 1. |
| 2026-09-17 | **The shared-tier leak on macOS, and a finding withdrawn** (`fa03e37`, `14515a3`, `07e029b`). One cause behind three symptoms, corrected twice: § 6 first said the leak was macOS-only (it is on Linux too), and **§ 13 was withdrawn outright** — its "controlled pair" differed in a second variable, so it proved nothing. The lesson kept: I raced my own install twice with `until docker exec … test -x`, which goes true MID-INSTALL, and produced two false findings from it. Installs now write a completion marker and the poll waits for that. |
| 2026-09-17 | **`check.pm` writes only the OS_CMD values it was given** (`823d50a`, `50ae69e`) — the darwin gap, verified on the VM. |
| 2026-09-17 | **The macOS portability pass — core 12 failures to zero** (`493c25c`…`a6872ae`). Every one of the twelve was a **defect in the test, not in the code under test**: fixture roots compared raw `mktemp` paths against resolved ones (macOS `/tmp` → `/private/tmp`), and `grep -c -- '-one$\|-two$'` used `\|`, which BSD grep does not read as alternation. `test.suite.fixture.make` was added as the canonical fixture root. One REAL defect among them: `user.create <name> password ""` created an account **with** a ShadowHash on macOS rather than a locked one — the darwin branch was inverted. This pass is what produced the portability card. |
| 2026-09-17 | **Card 2 delivered — F4/F5/F6, the guard that was discarded and the repair that made it worse** (`2e999ea`…`6c5a121`). `config validate required` told an operator to "repair with: config init.env"; measured on the VM that command made things **worse**, because `config.save` persists whatever is in the environment and the environment is populated from the config — a closed loop in which a value that falls out can never come back. Six `private.config.derive.*` getters give each required variable ONE named accessor, called from both the repair and the drift check so they cannot disagree. `config.init.env` now backs up all THREE files (backing up only `user.env` is precisely why `OOSH_PM` was lost), re-derives only what is EMPTY in this shell, names a deriver that returns nothing instead of dropping it, and **refuses to worsen** — it re-validates after the save and restores if the missing set grew. F4's verdict goes to **stderr**: `$(user get basehome)` captures `config.save`'s stdout, where a stray line once corrupted a generated git config. Two things found while building it: a deriver must have NO side effects (the first `OOSH_PM` one reached `oo pm.discover` → a real `sudo apt-get update`, and sat on a password prompt), and T83's first form asserted `config.save`'s stdout was empty when it has always carried `console.log` — narrowed to "the VERDICT must not be on stdout", which is the assertion that actually protects the caller. |
| 2026-09-17 | **Card 3 delivered — the config stops degrading** (`5731a9e`, `c1e3981`). A rewrite no longer drops a required variable the file already had. With cards 1 and 2 this closes the lifecycle: written correctly (1), reported when wrong (F4), repaired when incomplete (F5/F6), and now **kept** once correct. |
| 2026-09-17 | **The four package-manager maps agree** (`83b8f7e`). `pacman` and `pkg` lacked their non-interactive flags in `oo`'s copy, so an install on Arch or FreeBSD would have sat on a prompt. `T-PM-TABLES-AGREE` pins all four copies against each other rather than against a hand-typed list. |
| 2026-09-17 | **CONFIG_PATH — not reproduced, trigger removed** (`1436f80`). Research § 5 / § 7.3 reported `CONFIG_PATH` exported by a parent and **empty two processes down** on macOS. It did not reproduce: T85 pins the anchor into a grandchild and passes. Recorded as an honest non-reproduction rather than a fix, with the trigger removed anyway. |
| 2026-09-17 | **T50 runs its probe with `bash`** (`3266d2f`). `script -qec '<cmd>'` does not guarantee which shell runs the command; under dash the probe died on `source: not found`, never sourced `log` at all, and reported log's D4 guard broken in **every container** for a day. The probe is a file run as `bash <file>` now. The third of the three faults that produced the portability card. |
| 2026-09-17 | **The test-harness portability card delivered.** `test.suite portability.validate` joins `path.validate` and `this.anchor.validate` as a third sweep, and is the first one pointed at **test** code rather than production code — production portability is enforced by running the thing on the platforms, and nothing ran the harness anywhere new often enough to catch the three faults of 2026-09-17, all of which reported **working code** as broken. Two findings shaped it. First, a sweep is only as honest as its false-positive rate: the initial 7-rule table flagged 14 correct sites (`grep -E` with an escaped literal pipe, `stat -c` paired across a continued line, `date -r … \|\| date -d …`, `pgrep -P`, and `sed -i` inside a message string), and every one of those became a rule the sweep understands rather than a marker somebody has to write. Second, a gate that is permanently red is not a gate: `mktemp` is 118 sites and every one is *latent*, so the table carries a **severity** column and `mktemp` is an `advisory` that reports a per-file tally without failing. 22 genuine violations to 0. Two prerequisites first: `oo method.new` resolved `test.suite.portability.validate` to the **directory** `test/` (`${nameBody%%.*}` splits on the first dot) — now longest-existing-prefix, red-first with `T-METHOD-NEW-DOTTED-SCRIPT`; and `test.suite` had no `### new.method` marker, so the tool could not write into it at all. core 852 → 858 assertions, 1 intentional failure, canary silent. |
| 2026-09-17 | **The mktemp card filed, and its part (a) delivered** (`cf83fb8`…`53fe677`). Filing it exposed a defect in the sweep shipped an hour earlier: the `mktemp` rule was the bare pattern with no command-position anchor, so it counted the WORD — `\|\| { create.result 1 "mktemp failed"; … }` is the commonest line in the tree containing it — and over-reported its own advisory tally by **43%**, 119 against 82 real. That is the same false-positive class `sed -i` was anchored for in the same commit, left unfixed for `mktemp`; a validator reporting faults that are not there is a mild version of exactly what the sweep exists to stop, which is why it was worth fixing independently of whether any fixture is ever converted. Verified as a **SET**, not a count: the rule's matches and a hand-built list of real invocations agree exactly, 82 each, no misses, no extras — which also showed the card's own estimate of 83 was one too high, having counted a comment line. Part **(b)**, converting the 82 fixtures, is filed with its scope decision explicitly **NOT taken**: three options recorded, and the honest position is that nothing is red and the class has produced exactly one incident. |
