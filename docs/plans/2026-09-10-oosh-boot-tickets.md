...
| **`test.odocker` fails in every container** | Six assertions assert the HOST's `/var/dev/…/DockerWorkspaces` tree exists — ungated, unlike the docker-touching assertions beside them which are all behind `DOCKER_AVAILABLE`. They date from `3cfd931`; backlog item 8 gave the file its `core` category and score, and its plan said *"No platform run: odocker is off the install path"*, so nothing ran them in a container until 2026-09-17. `os platform.test ubuntu_24_04` now reports 7 failures (those 6 + the 1 intentional meta-test) on a clean tree. They want the same `[ -d "$ODOCKER_WORKSPACES" ]` skip the docker ones have. | [research § 14](../research/2026-09-17-config-save-bash32.md) |
