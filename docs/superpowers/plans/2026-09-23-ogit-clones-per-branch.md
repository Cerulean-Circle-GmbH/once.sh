...
3. **Getters answer on stdout and never call `create.result`** (they are consumed as `$(...)` and by tab completion; `create.result` inside a completion path breaks c2). Mutators `create.result N "msg"` then `return $(result)`. Mutators capture git's stderr via `private.ogit.git.run` and append ` — git: <reason>` to RESULT on failure (decided 2026-09-24).
...
- [x] Record in `sessions/agent.context.md`; add memory: "layout is clones per folder since <commit>; `ogit worktree.restore` reverses".

*Done 2026-09-25 (gates + migration):* three platform gates PASS on `7d181d2` — ubuntu_24_04 ×4 users, alpine_3_19 ×4 users, tart macOS Sequoia 15.7.3 (core 932/933, invariant 5/5); fixes found on the way: `6ff8739` (umask 002 in shared repos — FETCH_HEAD), `7d181d2` (T-PROMOTE-RESUME-3 order). Host migration: the first `sudo ./ogit worktree.remove` removed dev/ and then failed to clone from GitHub as root (no key) — dev/ recovered by hand as the user, `9d94932` (clone from main/ beside the worktree, swap only on success; T-OGIT-WORKTREE-REMOVE-OFFLINE), then prod converted; base setgid fixed; `oo profile.fix` (drop-in was never installed on this host); `oo checkout testing`; host invariant 5/5 PASS. Step 8 (restore round trip) NOT run: `worktree.restore` fetches from GitHub as root under sudo — same credential gap, refuses before touching anything; follow-up.

*Done 2026-09-24 (invariant + docs):* the migration runbook lives in [docs/ogit.md § Migrating a host from worktrees to clones](../../ogit.md#migrating-a-host-from-worktrees-to-clones) (9 steps + "If something goes wrong"). It runs the converters as `cd ~/oosh && sudo ./ogit worktree.remove "$base"` — `sudo` resets PATH, so `sudo ogit …` by name is not found on a host whose root PATH lacks oosh (not verified under sudo here — no password in the agent session). On this host (unmigrated) the invariant fails INVARIANT-1 (`dev prod` are worktrees) and INVARIANT-4 (base `root:dev` without setgid), passes 0/2/3 — expected until the runbook is run. The gates, push and migration boxes above stay open.
